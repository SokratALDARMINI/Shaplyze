...
class ClassifierMeasure(Measure, ABC):
    def __init__(self, df, X, A, Y, mode: Literal["replace", "drop"],classifier = LogisticRegression(), categorical_features = [], split_ratio = 0.33, seed = 0):
        self.classifier_base = ClassifierBase(classifier, df, X, A, Y, categorical_features=categorical_features, split_ratio=split_ratio, seed=seed)
        self.mode = mode
        self.X = X
        self.A = A
        self.Y = Y
...
class SurrogateMeasure(Measure):
    def __init__(self, df, X, A, Y, measure_class: type, mode:  Literal["replace", "drop"], categorical_features = [], classifier = LogisticRegression(), split_ratio = 0.33, seed = 0, **measure_kwargs):
        self.classifier_base =  ClassifierBase(classifier, df, features=X, sensitive_attribute=A, target_label=Y, categorical_features=categorical_features, split_ratio=split_ratio, seed=seed)
