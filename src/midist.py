...
Functions:
    mi(dist, header, Z, A): Mutual information I(Z; A) using feature names.
    ...
    def uni(dist, header, Z, A, B, eps=1e-7):
        """
           ...
           - Internally, the function:
             ...
             6. Attempts PID computation with ECOS first (for large support or multi-dimensional Z), then falls back to MOSEK.
           ...
           - Maintains original fallback logic: ECOS attempted first for large support; MOSEK is primary for small supports.
           - Does not alter distribution or solver parameters (max_iters, eps) from the original implementation.
           """
